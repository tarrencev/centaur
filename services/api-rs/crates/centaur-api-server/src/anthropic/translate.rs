    ...
    fn translate_codex_event(&mut self, value: &Value, out: &mut Vec<AnthropicStreamEvent>) {
        ...
        match event_type {
            Some("item.reasoning.textDelta" | "item.reasoning.summaryTextDelta") => {
                let text = terminal_payload_text(value);
                if !text.is_empty() {
                    self.emit_thinking_delta(&text, out);
                }
            }
            Some("item.agentMessage.delta") => {
                if let Some(FinalAnswerTextUpdate::Append(delta)) =
                    output_line_final_answer_text(value)
                {
                    let raw_delta = terminal_payload_text(value);
            ...
            Some("item.completed") => {
                if let Some(FinalAnswerTextUpdate::Replace(full_text)) =
                    output_line_final_answer_text(value)
                {
                    self.emit_replacement_text(&full_text, out);
                }
            }
            _ => {}
        }
